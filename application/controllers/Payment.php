...
class Payment extends CI_Controller
{
	public function __construct()
	{
		...
		if ($this->session->userdata('logged_in')) {
			...
			$this->session->set_flashdata('alert', ['type' => 'warning', 'message' => 'You Must Login First']);
	...
	public function input()
	{
		...
		if ($this->email->send()) {
			...
			$this->session->set_flashdata('alert', ['type' => 'success', 'message' => 'Email Berhasil Dikirim']);
		} else {
			// echo "<script>alert('Email gagal dikirim'); </script>";
			$this->session->set_flashdata('alert', ['type' => 'error', 'message' => 'Email Gagal Dikirim']);
		}

		// $this->session->unset_userdata('count');
		// echo "<script>alert('Data Sedang diproses tunggu beberapa saat'); </script>";
		$this->session->set_flashdata('alert', ['type' => 'success', 'message' => 'Data Sedang diproses tunggu beberapa saat']);
	...
	public function barcode($code)
	{
		...
		file_put_contents('assets/imgEvent/barcode/' . $code . '.jpg', $generator->getBarcode($code, $generator::TYPE_CODE_128));
